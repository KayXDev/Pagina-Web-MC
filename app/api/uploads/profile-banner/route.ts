  ...
  try {
    ...
    if (isCloudinaryEnabled()) {
      const uploaded = await uploadImageBuffer({
        ...
        publicId: `banner_${Date.now()}_${Math.random().toString(16).slice(2)}`,
      });
    ...
    const dir = path.join(process.cwd(), 'public', 'uploads', 'profile', 'banner');
    ...
    return NextResponse.json({ url: `/uploads/profile/banner/${filename}` });
  } catch (error: any) {
