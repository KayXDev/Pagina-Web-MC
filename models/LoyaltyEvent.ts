...
export interface ILoyaltyEvent {
  ...
  userId: string;
  orderId?: string;
  type: 'ORDER_EARNED' | 'ORDER_REDEEMED' | 'ADMIN_ADJUSTED' | 'ADMIN_SENT' | 'POINTS_CONVERTED_TO_BALANCE';
  points: number;
  amountSpent: number;
  currency: string;
  description: string;
...
const LoyaltyEventSchema = new Schema<ILoyaltyEvent>(
  {
    ...
    type: {
      type: String,
      enum: ['ORDER_EARNED', 'ORDER_REDEEMED', 'ADMIN_ADJUSTED', 'ADMIN_SENT', 'POINTS_CONVERTED_TO_BALANCE'],
