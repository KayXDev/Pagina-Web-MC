    ...
    <div ref={rootRef} className={`relative flex items-center ${className}`}>
      ...
      {open && (
        <div
          role="menu"
          aria-label={t(lang, 'lang.label')}
          className="absolute right-0 top-full mt-2 w-44 bg-black/95 border border-minecraft-grass/20 rounded-md overflow-hidden"
        >
          <div className="px-3 py-2 text-xs text-gray-400 border-b border-minecraft-grass/10">
            {t(lang, 'lang.label')}
          </div>
          <div className="py-1">
            {options.map((opt) => (
              <button
                ...
                className="w-full text-left px-3 py-2 text-sm text-gray-200 hover:bg-white/10 flex items-center justify-between"
              >
                <span>{opt.label}</span>
